def get_clinical_insights(user_input):
    ...
    if user_input['radius_mean'].iloc[0] > 18:
        insights.append({
            'feature': 'Radius Mean',
            'insight': 'Tumor radius exceeds the typical benign threshold (18mm), suggesting significant horizontal growth.',
            'severity': 'high'
        })
    ...
    if user_input['concave_points_mean'].iloc[0] > 0.1:
        insights.append({
            ...
            'insight': 'Extremely high concave points count, a strong indicator of malignant cell complexity.',
            'severity': 'high'
        })
    ...
    if user_input['area_worst'].iloc[0] > 1000:
        insights.append({
            'feature': 'Area Worst',
            'insight': 'The largest recorded area of the tumor is significantly high (>1000), which correlates with advanced stage development.',
            'severity': 'high'
        })
